...
void DrawBuffer::LinkTarget(RenderTarget& _renderTarget)
{
	...
	{
		if ((*it).GetDevice() == _renderTarget.GetDevice())
		{
			//TODO: Log a warning cause the device is already linked
			return;
		}
	}

	m_deviceLinks.push_back(BufferDeviceLink(_device, _physDevice))
}
...
void DrawBuffer::UpdateBuffers()
{
	...
	//TODO: Decide if the vertex data buffers should be resized when we add data
	//TODO: If not, remove the rest of the comment
	...
	//m_modelData = new glm::mat4x4[m_instanceCount];
