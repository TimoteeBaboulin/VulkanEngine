...
void Renderer::InitContext(ContextInfo& _info, std::vector<const char*> requiredExtensions)
{
	...
	m_renderTargets.push_back(new RenderTarget(3, _info.windowHandle, m_instance, m_cameras[0]));
	m_deviceManager = new RendererDeviceManager(m_renderTargets[0]);
	m_deviceManager->Init(m_instance);
	m_device = m_deviceManager->GetDevice();
	CreateUboDescriptorSetLayout();
	m_renderTargets[0]->LinkDeviceManager(m_deviceManager);
