	...
	: m_textureList(), m_deviceBridge(_deviceData, *_material, this)
{
	...
	m_deviceBridge.Initialise();
}

Moonlit::Renderer::DrawBuffer::~DrawBuffer()
{
}

void Moonlit::Renderer::DrawBuffer::RemoveMeshInstance(uint32_t _instanceId)
{
...
uint32_t Moonlit::Renderer::DrawBuffer::AddMeshInstance(MeshHandle _mesh, std::vector<TextureHandle> _textures, glm::mat4x4 _model)
{
	...
	intptr_t meshHandle = reinterpret_cast<intptr_t>(&(*_mesh));
...
void Moonlit::Renderer::DrawBuffer::UpdateInstanceModel(uint32_t _instanceId, glm::mat4x4 _model)
{
	auto it = std::find_if(m_meshInstances.begin(), m_meshInstances.end(),
		[_instanceId](const InstanceData& _data)
		{
...
void Moonlit::Renderer::DrawBuffer::UpdateInstanceMesh(uint32_t _instanceId, MeshHandle _mesh)
{
	...
	size_t index = std::distance(m_meshInstances.begin(), it);
	currentModel = m_modelData[index * 2];
	...
	intptr_t newMeshHandle = reinterpret_cast<intptr_t>(&(*_mesh));
	...
	InsertMeshInstance(newMeshHandle, currentModel, _instanceId);

	SetDeviceLinkDirty();
}

void Moonlit::Renderer::DrawBuffer::RenderBuffer(RenderTarget& _target, vk::CommandBuffer& _cmd, std::string _renderPass)
{
	if (m_meshEntries.size() == 0)
	{
		// LOG_INFO("[DrawBuffer::RenderBuffer] No meshes to render in this draw buffer, skipping.");
		return;
	}
	...
	{
		// LOG_INFO("[DrawBuffer::RenderBuffer] The material used in this draw buffer doesn't include the requested subpass, skipping.");
...
void Moonlit::Renderer::DrawBuffer::InsertMesh(MeshHandle _mesh)
{
	m_meshEntries.push_back(MeshEntry{
		reinterpret_cast<intptr_t>(&(*_mesh)),
		0
		});
	...
	{
		m_indexData.emplace_back((*_mesh).indices[i]);
	}

	// Copy mesh data into the mesh entry
	// Deep copy to avoid making a call to a deleted mesh
...
void Moonlit::Renderer::DrawBuffer::RemoveMeshInstance(std::vector<InstanceData>::iterator _instanceIt)
{
	// Remove model data
	auto modelIt = m_modelData.begin() + (std::distance(m_meshInstances.begin(), _instanceIt) * 2);
	m_modelData.erase(modelIt, modelIt + 2);

	// Remove mesh instance after making sure we found the right meshEntry
	auto meshIt = FindMeshEntry((*_instanceIt).MeshHandle);
	m_meshInstances.erase(_instanceIt);

	// Decrease instance count
	// If it reaches 0, remove the mesh entry and its data
	if (meshIt == m_meshEntries.end())
	{
		LOG_ERROR("Trying to remove mesh instance that exists but without a corresponding mesh entry");
		throw std::runtime_error("Mesh entry not found!");
...
void Moonlit::Renderer::DrawBuffer::InsertMeshInstance(intptr_t _meshHandle, glm::mat4x4 _model, uint32_t _instanceId)
{
	InstanceData instanceData;
	instanceData.Id = _instanceId;
	instanceData.MeshHandle = _meshHandle;

	m_meshInstances.emplace_back(instanceData);
	m_modelData.emplace_back(_model);
	m_modelData.emplace_back(glm::inverse(_model));

	auto meshIt = FindMeshEntry(_meshHandle);
	if (meshIt == m_meshEntries.end())
	{
		LOG_ERROR("Trying to insert mesh instance for a mesh that doesn't exist in the draw buffer");
		throw std::runtime_error("Mesh entry not found!");
	}
...
void Moonlit::Renderer::DrawBuffer::RemoveMesh(std::vector<MeshEntry>::iterator _meshIt)
{
...
std::vector<Moonlit::Renderer::MeshEntry>::iterator Moonlit::Renderer::DrawBuffer::FindMeshEntry(intptr_t _handle)
{
...
std::vector<uint16_t> Moonlit::Renderer::DrawBuffer::GetTextureIndices(std::vector<TextureHandle>& _images)
{
	...
	{
		intptr_t texHandle = reinterpret_cast<intptr_t>(&(**it));
...
uint16_t Moonlit::Renderer::DrawBuffer::InsertTexture(TextureHandle& _image)
{
	...
	newEntry.TextureHandle = reinterpret_cast<intptr_t>(&(*_image));
...
void Moonlit::Renderer::DrawBuffer::RemoveTexture(uint16_t _index)
{
	m_textureList.erase(m_textureList.begin() + _index);
}

std::vector<Moonlit::Renderer::TextureSlot>::iterator Moonlit::Renderer::DrawBuffer::FindTexture(intptr_t _texHandle)
{
...
void Moonlit::Renderer::DrawBuffer::SetDeviceLinkDirty()
{
