...
std::vector<char> Moonlit::FileHelper::ReadFile(std::string _path)
{
	...
	if (!stream.is_open())
	{
		Moonlit::Debug::Logger::LogWarning("Failed to open file: " + _path);
		return buffer;
	}
...
std::vector<std::string> Moonlit::FileHelper::ListFilesInDirectory(const std::string& _directoryPath)
{
